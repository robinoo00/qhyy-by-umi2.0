...
export default class {
    ...
    getDrawArr() {
        ...
        if(this.放大){
            if(this.scale < 1){//缩小
                this.K线显示柱条数 = this.K线显示柱条数 + Math.floor(2 / this.scale);
                if (this.K线显示柱条数 > this.data.length) {
                    this.K线显示柱条数 = this.data.length
                }
            }else if(this.scale > 1){//放大
                this.K线显示柱条数 = this.K线显示柱条数 - Math.floor(1.9 * this.scale)
                if (this.K线显示柱条数 < 1) {
                    this.K线显示柱条数 = 1
                }
            }
            if(this.data.length < this.K线显示柱条数){
                this.开始条数 = 0
            }else{
        ...
        //拖动
        const diff_start = this.开始条数 - this.拖动条数
        const diff_end = this.结束条数 - this.拖动条数
        if(this.拖动条数 >= 0){//向右拖动
            // console.log('向右拖动')
            this.开始条数 = diff_start <= 0 ? 0 : diff_start
            this.结束条数 = diff_start <= 0 ? this.K线显示柱条数 : diff_end
        }else{//向左拖动
            // console.log('向左拖动')
            this.开始条数 = diff_end >= this.data.length ? this.data.length - this.K线显示柱条数 : diff_start
            this.结束条数 = diff_end >= this.data.length ? this.data.length : diff_end
        }
        const re = this.data.slice(this.开始条数, this.结束条数)
        console.log('开始条数', this.开始条数)
        console.log('结束条数', this.结束条数)
        if (re.length != this.K线显示柱条数) {
    ...
    * */
    ...
    getTimeArr() {
        ...
        for (let i = 0; i <= this.时间个数; i++) {
            let index = i * diff === data.length ? data.length - 1 : Math.ceil(i * diff)
            if(data[index]){
